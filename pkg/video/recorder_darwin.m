...
- (BOOL)recordWithScreenCaptureKitToURL:(NSURL *)url duration:(NSTimeInterval)duration error:(NSError **)error API_AVAILABLE(macos(12.3)) {
    ...
    if (!content) {
        NSLog(@"[Recorder] ScreenCaptureKit shareable content unavailable: %@", contentError);
        if (error) {
    ...
    if (!display) {
        NSLog(@"[Recorder] No display selected from content: %@", content.displays);
        if (error) {
    ...
    if (![self.stream addStreamOutput:self.writer type:SCStreamOutputTypeScreen sampleHandlerQueue:self.streamQueue error:&outputError]) {
        NSLog(@"[Recorder] Failed to add stream output: %@", outputError);
        if (error) {
    ...
    if (startError) {
        NSLog(@"[Recorder] Stream start failed: %@", startError);
        if (error) {
            *error = startError;
        }
        return NO;
...
static char *recorder_copy_error_string(NSError *error, NSString *fallback) {
    NSString *message = fallback ?: @"recording failed";
    if (error) {
        NSString *desc = error.localizedDescription ?: message;
        NSString *domain = error.domain ?: @"";
        NSInteger code = error.code;
        if ([domain isEqualToString:SCStreamErrorDomain] && code == SCStreamErrorUserDeclined) {
            message = [NSString stringWithFormat:@"SCREEN_RECORDING_PERMISSION_REQUIRED:%@", desc];
        } else if ([domain isEqualToString:AVFoundationErrorDomain] && code == AVErrorApplicationIsNotAuthorizedToUseDevice) {
            message = [NSString stringWithFormat:@"SCREEN_RECORDING_PERMISSION_REQUIRED:%@", desc];
        } else {
            NSString *domainLabel = domain.length > 0 ? domain : @"unknown";
            message = [NSString stringWithFormat:@"%@ (%@ %ld)", desc, domainLabel, (long)code];
        }
    }
    return recorder_copy_cstring(message);
}
...
int recorder_record_screen(const char *path, double duration, char **error_out) {
    @autoreleasepool {
        ...
        if (!success) {
            if (error_out) {
                *error_out = recorder_copy_error_string(error, @"recording failed");
            }
            return -1;
        }
        return 0;
